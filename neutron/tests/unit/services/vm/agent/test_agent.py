...
from neutron.vm.agent import agent
...
class TestVMService(base.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.mock_ipwrapper_p = mock.patch.object(neutron.agent.linux.ip_lib,
                                                  'IPWrapper')
    ...
    def test_create_destroy_namespace_agent(self):
        self.agent.create_namespace_agent(self.port)
        ...
        self.agent.destroy_namespace_agent(self.port_id)
    ...
    def test_create_rpc_proxy_wrong_port_id(self):
        func = lambda: self.agent.create_rpc_proxy(
            self.port_id, 'topic=src_topic,server=src_server',
    ...
    def test_create_destroy_rpc_proxy_send(self):
        ...
        with mock.patch('oslo.messaging.proxy.get_proxy_server'
            ...
            mock_get_proxy_server.assert_called_once_with(
                ...
                mock_transport, dst_target, None, executer=mock.ANY)
            mock_instance.start.assert_called_once_with()

            self.agent.destroy_rpc_proxy(self.port_id, proxy_id)
    ...
    def test_create_destroy_rpc_proxy_receive(self):
        ...
        with mock.patch('oslo.messaging.proxy.get_proxy_server'
            ...
            mock_get_proxy_server.assert_called_once_with(
                ...
                mock_transport, src_target, None, executer=mock.ANY)
            mock_instance.start.assert_called_once_with()
    ...
    def test_create_destroy_rpc_proxy(self):
        ...
        with mock.patch('oslo.messaging.proxy.get_proxy_server'
            ...
            proxy_id_send = self.agent.create_rpc_proxy(
                self.port_id, 'topic=src_topic_send,server=src_server_send',
            ...
            self.agent.destroy_rpc_proxy(self.port_id, proxy_id_send)
            self.agent.destroy_namespace_agent(self.port_id)
    ...
    def _test_create_destroy_rpc_namespace_proxy_direction(self, direction):
        ...
        ns_proxy_id = self.agent.create_rpc_namespace_proxy(
            self.port_id, src_unix_target, dst_target, direction)
        kwargs = {
            'src_unix_target': src_unix_target,
        ...
        self.agent.destroy_rpc_namespace_proxy(self.port_id, ns_proxy_id)
    ...
    def test_create_destroy_rpc_namespace_proxy(self):
        ...
        src_unix_target_send = 'topic=src_topic_send,server=src_server_send'
        dst_target_send = 'topic=dst_topic_send,server=dst_server_send'
        direction_send = 'send'
        ns_proxy_id_send = self.agent.create_rpc_namespace_proxy(
            self.port_id, src_unix_target_send, dst_target_send,
            direction_send)
        kwargs_send = {
            'src_unix_target': src_unix_target_send,
            'dst_target': dst_target_send,
            'direction': direction_send,
        }

        src_unix_target_recv = 'topic=src_topic_recv,server=src_server_recv'
        dst_target_recv = 'topic=dst_topic_recv,server=dst_server_recv'
        direction_recv = 'receive'
        self.agent.create_rpc_namespace_proxy(
            self.port_id, src_unix_target_recv, dst_target_recv,
            direction_recv)
        kwargs_recv = {
            'src_unix_target': src_unix_target_recv,
            'dst_target': dst_target_recv,
            'direction': direction_recv,
        }

        self.agent.destroy_rpc_namespace_proxy(self.port_id, ns_proxy_id_send)
        self.agent.destroy_namespace_agent(self.port_id)
