...
from neutron.services.l3_router.vrouter.agents.cfg_agent import cfg_exceptions
from neutron.services.l3_router.vrouter.agents.cfg_agent.device_drivers import(
    driver_mgr)
from neutron.services.l3_router.vrouter.agents.cfg_agent.device_drivers import(
    service_driver_base)
...
class RoutingPluginApi(n_rpc.RpcProxy):
    """RoutingServiceHelper(Agent) side of the  routing RPC API."""

    BASE_RPC_API_VERSION = '1.1'

    def __init__(self, topic, host):
        super(RoutingPluginApi, self).__init__(
...
class RoutingServiceDriver(service_driver_base.ServiceDriverBase):
    def __init__(self, host, conf, cfg_agent):
        super(RoutingServiceDriver, self).__init__()
        self.conf = conf
        self.cfg_agent = cfg_agent
        self.context = n_context.get_admin_context_without_session()
        self.plugin_rpc = RoutingPluginApi(topics.L3PLUGIN, host)
        ...
        self.fullsync = True
        self.topic = '%s.%s' % (topics.CFG_AGENT_L3_ROUTING, host)
    ...
    def process(self, device_ids=None, removed_devices_info=None):
        try:
