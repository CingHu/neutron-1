...
from neutron.services.l3_router.vrouter.agents.cfg_agent import device_status
from neutron.services.l3_router.vrouter.utils import stevedoreutils
...
class DeviceManagementApi(n_rpc.RpcProxy):
    """Agent side of the device manager RPC API."""

    BASE_RPC_API_VERSION = '1.0'

    def __init__(self, topic, host):
        super(DeviceManagementApi, self).__init__(
            topic=topic, default_version=self.BASE_RPC_API_VERSION)
        self.host = host

    def report_non_responding_hosting_devices(self, context, hd_ids=None):
    ...
    def register_for_duty(self, context):
        ...
        return self.call(context,
                         self.make_msg('register_for_duty', host=self.host),
                         topic=self.topic)


class ServiceManager(object):
    def __init__(self, service_drivers):
        super(ServiceManager, self).__init__()
        services = []
        for driver in self._conf.service_drivers:
            try:
                services += stevedoreutils.import_object(driver)
            except ImportError as e:
                LOG.exception(_("Error in loading service driver %s(driver)s. "
                                "Reason:%(reason)s"),
                         {'driver': driver, 'reason': e})
                raise SystemExit("config agent can not load service driver")
        self._service_drivers = services

    def process(self, device_ids, removed_devices_info):
        for service in self._service_drivers:
            service.process(device_ids, removed_devices_info)

    def collect_state(self, configurations):
        ret = {}
        for service in self._service_drivers:
            ret.update(service.collect_state(configurations))
        return ret


class ConfigAgent(manager.Manager):
    """Config Agent.
    ...
    OPTS = [
        cfg.IntOpt('rpc_loop_interval', default=10,
                   help=_("Interval when the process_services() loop "
                          ...
                          "resources.")),
        cfg.StrOpt('config_agent_drivers',
                   default=['routing_serivce'],
                   help=_("list of services for config agent.")),
    ...
    def __init__(self, host, conf=None):
        self._conf = conf or cfg.CONF
        self._context = n_context.get_admin_context_without_session()
        self._dev_status = device_status.DeviceStatus()

        self._initialize_rpc(host)
        self._service_manager = ServiceManager(self._conf.service_drivers)
        self._start_periodic_tasks()
        super(ConfigAgent, self).__init__(host=self._conf.host)

    def _initialize_rpc(self, host):
        self._devmgr_rpc = DeviceManagementApi(topics.L3PLUGIN, host)

    def _start_periodic_tasks(self):
        self.loop = loopingcall.FixedIntervalLoopingCall(self.process_services)
        self.loop.start(interval=self._conf.rpc_loop_interval)

    def after_start(self):
        LOG.info(_("Config agent started"))
    ...
    ## Main orchestrator ##
    @lockutils.synchronized('config-agent', 'neutron-')
    def process_services(self, device_ids=None, removed_devices_info=None):
        ...
        # added in future
        self._service_manager.process(device_ids, removed_devices_info)
...
class ConfigAgentWithStateReport(ConfigAgent):

    def __init__(self, host, conf=None):
        self.state_rpc = agent_rpc.PluginReportStateAPI(topics.PLUGIN)
        self.agent_state = {
            'binary': 'neutron-config-agent',
            'host': host,
            'topic': topics.CONFIG_AGENT,
            'configurations': {},
            'start_flag': True,
            'agent_type': constants.AGENT_TYPE_CFG}
        report_interval = cfg.CONF.AGENT.report_interval
        self.use_call = True
        self._initialize_rpc(host)
        self._agent_registration()
        super(ConfigAgentWithStateReport, self).__init__(host=host, conf=conf)
        if report_interval:
    ...
    def _agent_registration(self):
        ...
        for attempts in xrange(MAX_REGISTRATION_ATTEMPTS):
            ...
            self._send_agent_report(self.agent_state, context)
            res = self._devmgr_rpc.register_for_duty(context)
            if res is True:
    ...
    def _report_state(self):
        ...
        configurations = {}
        configurations.update(self._service_manager.collect_state(
        ...
        self._send_agent_report(self.agent_state, self._context)

    def _send_agent_report(self, report, context):
...
def main(manager='neutron.services.l3_router.vrouter.agents.'
         'cfg_agent.cfg_agent.ConfigAgentWithStateReport'):
    conf = cfg.CONF
    conf.register_opts(ConfigAgent.OPTS)
