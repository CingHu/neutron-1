...
# TODO
class CiscoRouterApplianceDriver(
        l3_router_appliance_db.L3RouterNotifierDriver):
    ...
    def remove_router_interface_db(self, driver_context):
        ...
        port_db = driver_context.aux['port_db']
        self._add_type_and_hosting_device_info(
...
class L3RouterApplianceDBMixin(extraroute_db.ExtraRoute_db_mixin):
    ...
    def create_floatingip(
        ...
        driver = self._driver
        driver_context = L3DriverContext(context)
        with context.session.begin(subtransactions=True):
            info = super(L3RouterApplianceDBMixin, self).create_floatingip(
                context, floatingip)
            driver_context.aux = info
            if info['router_id']:
