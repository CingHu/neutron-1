...
class L3RouterCfgRpcCallbackMixin(l3_rpc_base.L3RpcCallbackMixin):
    """Mixin for Cisco cfg agent rpc support in L3 routing service plugin."""
    def __init__(self, l3plugin=None):
        if l3plugin is not None:
            self._l3plugin = l3plugin

    def sync_routers(self, context, host, router_ids=None,
